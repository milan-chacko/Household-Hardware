...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`http://localhost:5555/checkout?userId=${userId}`);
    ...
    const fetchCartItems = async () => {
      try {
        const cartResponse = await axios.get(`http://localhost:5555/cart/${userId}`);
        setCartItems(cartResponse.data);

        const calculatedTotals = cartResponse.data.reduce((acc, item) => ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.paymentMethod === 'Cash on Delivery' || formData.paymentMethod === 'Online Payment') {
      finalAmount += 50;
    }
